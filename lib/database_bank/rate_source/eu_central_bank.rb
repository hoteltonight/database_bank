          ...
          def self.fetch_rates
            ...
            rate_data.each do |rate|
              ...
              if DatabaseBank.currencies.include? to_currency
                rates << { sourced_at: sourced_at, rate_source: NAME, from_currency: BASE_CURRENCY, to_currency: to_currency, rate: rate['rate'].to_f }
                rates << { sourced_at: sourced_at, rate_source: NAME, from_currency: to_currency, to_currency: BASE_CURRENCY, rate: 1.0 / rate['rate'].to_f }
              end
            end
